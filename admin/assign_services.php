<?php
  ...
  if(isset($_POST['save'])){
    ...
    for($i=0;$i<count($sid);$i++){
      $svid=$sid[$i];
      $ret=mysqli_query($con,"insert into tblinvoice(Customers_ID,Services_Id,BillingId,invoicefrom) values('$uid','$svid','$invoiceid','WALK IN CLIENT');");
    ...
    <tbody>
      ...
      while ($row=mysqli_fetch_array($ret)) {
        ...
        <tr> 
          ...
          <td><input type="checkbox" name="sids[]" value="<?php  echo $row['Services_ID'];?>" ></td>
        </tr>   
        <?php 
        $cnt=$cnt+1;
      }?>
